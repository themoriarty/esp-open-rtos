...
typedef struct {
	...
	int RootCALen;				///< Pointer to a string defining the Root CA file (full file, not path)
	unsigned char *DeviceCert;
	int DeviceCertLen;			///< Pointer to a string defining the device identity certificate file (full file, not path)
	unsigned char *DevicePrivateKey;
	int DevicePrivateKeyLen;	///< Pointer to a string defining the device private key file (full file, not path)
