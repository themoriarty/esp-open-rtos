...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	ret = mbedtls_x509_crt_parse(&cacert, params.RootCA, params.RootCALen);
	if (ret < 0) {
	...
	ret = mbedtls_x509_crt_parse(&clicert, params.DeviceCert, params.DeviceCertLen);
	if (ret != 0) {
		ERROR(" failed\n  !  mbedtls_x509_crt_parse returned -0x%x\n\n", -ret);
		return ret;
	}
	//DEBUG("  . Loading the device private key ...");
	ret = mbedtls_pk_parse_key(&pkey, params.DevicePrivateKey, params.DevicePrivateKeyLen, NULL, 0);
	if (ret != 0) {
